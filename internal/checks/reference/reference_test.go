...
func runCtx(mode checks.Mode, topo config.Topology) *checks.RunContext {
...
func TestRunsIdenticallyInEveryMode(t *testing.T) {
	...
	for _, mode := range checks.AllModes {
		t.Run(string(mode), func(t *testing.T) {
			res, err := c.Run(context.Background(), runCtx(mode, config.TopologyNSX))
			if err != nil {
				t.Fatalf("Run: %v", err)
			}
			if len(res) != 1 {
			...
			if got := r.Observed.Data["topology"]; got != string(config.TopologyNSX) {
				t.Errorf("Observed.Data[topology] = %v, want %v", got, config.TopologyNSX)
			}
		})
	}
}

func TestGradesEveryTopology(t *testing.T) {
	t.Parallel()
	c := &reference.TopologyRecognised{KnownCheckCount: 1}
	...
	tests := []struct {
		...
		{"nsx", config.TopologyNSX, results.StatusPass},
		{"nsx with alb", config.TopologyNSXALB, results.StatusPass},
		{"vds with alb", config.TopologyVDSALB, results.StatusPass},
		{"vds with haproxy", config.TopologyVDSHAProxy, results.StatusPass},
		{"nsx vpc", config.TopologyNSXVPC, results.StatusPass},
		{"unknown topology fails rather than being ignored", config.Topology("openshift"), results.StatusFail},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if res[0].Status != tt.want {
				t.Errorf("status = %s, want %s", res[0].Status, tt.want)
			}
		})
	}
}
...
func TestObservationIsMachineComparable(t *testing.T) {
	...
	res, err := c.Run(context.Background(), runCtx(checks.ModeSnapshot, config.TopologyVDSALB))
	if err != nil {
		t.Fatal(err)
	}
	r := res[0]
...
// directly cannot be golden-tested and will make the whole report nondeterministic.
func TestUsesInjectedClock(t *testing.T) {
	t.Parallel()
	c := &reference.TopologyRecognised{}
	rc := runCtx(checks.ModePreflight, config.TopologyNSX)
...
func TestMetaIsWellFormed(t *testing.T) {
	...
	for _, topo := range config.AllTopologies {
		if !m.AppliesTo(topo) {
			t.Errorf("check should apply to every topology, but not %s", topo)
		}
	}
	if m.Remediation == "" {
